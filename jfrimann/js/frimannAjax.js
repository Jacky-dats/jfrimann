...
function frimannAjax(configPath,params,onSuccess,onError) {
	//var sign = hex_md5(hex_md5(callb+kt+password+union+method+lang+appid+appkey));
    $.getJSON(configPath, function(data) {
    	params.lang = data[0].lang;
    	params.union = data[0].union;
  		params.appid = data[0].appid;
  		params.appkey = data[0].appkey;
		var sign = '';
		for(var i in params){
	        sign += params[i];
	    }
	    var callb = "_fc" + Math.round(Math.random() * 10000);
	    params.sign = hex_md5(hex_md5(callb + sign));
		$.ajax({
		    url: data[0].url,
		    dataType: 'jsonp',
		    //jsonp: "callbackparam",
